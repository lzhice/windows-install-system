...
static const DUI_MSGMAP_ENTRY* DuiFindMessageEntry(const DUI_MSGMAP_ENTRY* lpEntry,
                                                   TNotifyUI& msg) {
...
void CNotifyPump::NotifyPump(TNotifyUI& msg) {
  ...
  if (!msg.sVirtualWnd.IsEmpty()) {
    for (std::map<CDuiString, LPVOID>::iterator it = m_VirtualWndMap.Begin();
         it != m_VirtualWndMap.End(); it++) {
...
void CWindowWnd::SetIcon(UINT nRes) {
  HICON hIcon =
      (HICON)::LoadImage(CPaintManagerUI::GetInstance(), MAKEINTRESOURCE(nRes), IMAGE_ICON,
