...
struct Unzipper::Impl {
 ...
 public:
  ...
  bool extractCurrentEntryToFile(ZipEntry& entryinfo, const std::wstring& fileName) {
    ...
    if (!PathContainFilename(fileName)) {
  ...
  int extractToFile(const std::wstring& filename, ZipEntry& info) {
    ...
    if (!MakeDir(GetParentDir(filename))) {
      return UNZ_ERRNO;
    }
 ...
 public:
  ...
  bool initWithRawData(void* data, unsigned int size) {
    if (data && size > 0) {
      m_zipmem.base = (char*)malloc(size * sizeof(char));
      memcpy(m_zipmem.base, (char*)data, size);
      m_zipmem.size = size;
    }
  ...
  bool extractAll(const std::wstring& destination,
    ...
    for (; it != entries.end(); ++it) {
      ...
      if (this->extractCurrentEntryToFile(*it, alternativeName)) {
        if (cb) {
          cb(alternativeName);
        }
      }
...
Unzipper::Unzipper(void* data, unsigned int size)
    : m_ibuffer(*(new std::stringstream()))  // not used but using local
                                             // variable throws exception
    ...
    , m_usingRawData(true)
    , m_impl(new Impl(*this)) {
  if (!m_impl->initWithRawData(data, size)) {
    release();
    throw EXCEPTION_CLASS("Error loading zip in memory!");
  }
...
bool Unzipper::extract(const std::wstring& destination, std::function<void(std::wstring)> cb) {
  return m_impl->extractAll(destination, std::map<std::wstring, std::wstring>(), cb);
}
