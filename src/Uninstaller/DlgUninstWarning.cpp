...
DlgUninstWarning::DlgUninstWarning(const CDuiString& title, const CDuiString& msg, bool hasCancelButton)
    : pBtnClose_(NULL)
    , pBtnOK_(NULL)
    , pBtnCancel_(NULL)
    , pLblTitle_(NULL)
...
DlgUninstWarning::~DlgUninstWarning() {}

WarningDialogClicked DlgUninstWarning::ShowWarning(HWND hParent,
                                             const CDuiString& title,
                                             const CDuiString& msg,
                                             bool hasCancleButton) {
  WarningDialogClicked ret = WarningDialogClicked::UNKNOWN;
  DlgUninstWarning dlg_warning(title, msg, hasCancleButton);
  if (dlg_warning.Create(hParent,
                         PARSE(UninstallerConfig::Instance()->GetCoreCfg().productName).c_str(),
...
CDuiString DlgUninstWarning::GetSkinFile() {
  return TEXT("warn.xml");
}

LPCTSTR DlgUninstWarning::GetWindowClassName() const {
  return TEXT("INSTALLER_WARNING_DIALOG");
}

void DlgUninstWarning::Notify(TNotifyUI& msg) {
...
LRESULT DlgUninstWarning::ResponseDefaultKeyEvent(WPARAM wParam) {
  if (wParam == VK_RETURN || wParam == VK_ESCAPE)
    return 0;
  return WindowImplBase::ResponseDefaultKeyEvent(wParam);
}

void DlgUninstWarning::InitWindow() {
